...
function App() {
  ...
  const [selectedMakeup, setSelectedMakeup] = useState<MakeupType>("foundation");
  ...
  async function searchProductImages(query: string) {
    try {
      const response = await fetch("https://google.serper.dev/images", {
        ...
        body: JSON.stringify({
          q: `${query} ${selectedMakeup === "custom" ? customMakeupType : selectedMakeup} makeup product`,
        }),
      });
  ...
  async function processFoundations(responseText: string) {
    ...
    const parts = responseText.split(new RegExp(`## Recommended ${makeupTitle}`, "i"));
    const analysisSection = parts[0] || "";
    setAnalysisText(analysisSection);

    // Extract makeup recommendations
    const makeupLines = responseText.split("\n").filter(line => line.trim().startsWith("- ") && line.includes("Brand:"));

    const processed = await Promise.all(
      makeupLines.map(async line => {
        const match = line.match(/-\s*(.*?),\s*Shade:\s*(.*?),\s*Brand:\s*(.*)/);
  ...
  function getMakeupTitle(type: MakeupType): string {
    switch (type) {
      ...
      case "blush":
        return "Blushes";
      case "custom":
        return customMakeupType ? customMakeupType.charAt(0).toUpperCase() + customMakeupType.slice(1) + "s" : "Products";
      default:
        return "Products";
    }
  }
      ...
      <div className='content-wrapper'>
          ...
          {imgSrc && (
            <div className='preview-wrapper flex items-center flex-col'>
              ...
              <span className='mt-2 text-center text-pink-500'>
                ✨ Use natural lighting and a clear, close-up photo of your face for the most accurate makeup matches! ✨
              </span>
            </div>
          )}
        </div>
        ...
        <div className='makeup-selector-container'>
          ...
          <div className='makeup-buttons'>
            <button className={`makeup-button ${selectedMakeup === "foundation" ? "active" : ""}`} onClick={() => setSelectedMakeup("foundation")}>
              <span className='makeup-icon'>💄</span>
              Foundation
            </button>
            <button className={`makeup-button ${selectedMakeup === "concealer" ? "active" : ""}`} onClick={() => setSelectedMakeup("concealer")}>
              <span className='makeup-icon'>🖌️</span>
              Concealer
            </button>
            <button className={`makeup-button ${selectedMakeup === "blush" ? "active" : ""}`} onClick={() => setSelectedMakeup("blush")}>
              <span className='makeup-icon'>🌸</span>
              Blush
            </button>
            <button className={`makeup-button ${selectedMakeup === "custom" ? "active" : ""}`} onClick={() => setSelectedMakeup("custom")}>
              <span className='makeup-icon'>✨</span>
              Custom
            </button>
          </div>
          ...
          {selectedMakeup === "custom" && (
            <div className='custom-makeup-input-container'>
              ...
              <p className='custom-makeup-disclaimer'>
                <span className='disclaimer-icon'>📸</span> For best results, take a close-up picture focused on the relevant area (e.g., for
        ...
        <div className='input-container flex justify-center items-center'>
          <input type='text' value={country} onChange={e => setCountry(e.target.value)} placeholder='Enter your country' className='country-input' />
          <button onClick={run} className='analyze-button' disabled={isLoading}>
            {isLoading ? "Analyzing..." : `Analyze for ${getMakeupTitle(selectedMakeup)}`}
          </button>
        </div>
        ...
        {aiResponse && (
          <div className='ai-response-container'>
            ...
            <ReactMarkdown
              ...
              components={{
                h1: ({ children }) => <h2 className='ai-response-header'>{children}</h2>,
                h2: ({ children }) => <h3 className='ai-response-subheader'>{children}</h3>,
                p: ({ children }) => <p className='ai-response-paragraph text-xs md:text-base'>{children}</p>,
                strong: ({ children }) => <strong className='ai-response-bold'>{children}</strong>,
              }}
            />
            ...
            {foundations.length > 0 && (
              <>
                <h3 className='ai-response-subheader text-3xl font-medium mb-8 text-rose-500 text-center tracking-wide'>
                  {getMakeupIcon(selectedMakeup)} Recommended {getMakeupTitle(selectedMakeup)} available in{" "}
                ...
                <div className='foundation-grid flex flex-wrap justify-center gap-8 px-4'>
                  {foundations.map((foundation, index) => (
                    <a key={index} href={`https://www.google.com/search?q=${foundation.brand + foundation.name + foundation.shade}`} target='_blank'>
                      <div className='foundation-card relative bg-gradient-to-br from-pink-50 to-rose-50 rounded-xl p-6 w-80 shadow-md hover:shadow-lg transition-all duration-300 border border-rose-100'>
                        <div className='makeup-type-label'>{selectedMakeup === "custom" ? customMakeupType : selectedMakeup}</div>
                        <div className='foundation-info mb-4'>
                          <p className='text-xl font-semibold text-rose-700 mb-3 tracking-tight'>{foundation.name}</p>
                          <div className='space-y-2 text-rose-600'>
                            <p className='text-sm uppercase tracking-wide font-medium text-rose-500'>{foundation.brand}</p>
                            <p className='text-sm text-rose-400'>Shade: {foundation.shade}</p>
                          </div>
                        </div>
                        {foundation.imageUrl && (
